#!/usr/bin/env python3
"""
gw29_extract.py - extractor for the GW-29 / TP-127 layout family.

Both GW-series rigs (GWDC operator, RBL wells, REB field) and TP-127
(DAD#1 well, DRAA DAOUI field) use an identical French-language template.
...
def parse_gw29(source):
    wb = load_workbook(source, data_only=True)
    ws = wb.active
    L = _build_merged_lookup(ws)

    header = {}
    header["well_name"]  = _strip_prefix(_clean(_cell(ws, 2, 2, L)), "Puits", "Well")
    header["rig_name"]   = _strip_prefix(_clean(_cell(ws, 2, 9, L)), "RIG", "APPAREIL", "Appareil")
    header["field_name"] = _clean(_cell(ws, 2, 5, L))
    header["well_class"] = ""
    header["date"]       = _date_parse(_cell(ws, 1, 21, L))
    day_raw = _cell(ws, 1, 26, L)
    header["day_number"] = _int(day_raw) if day_raw is not None else None
    header["bop_test"]      = _date_parse(_cell(ws, 37, 15, L))
    header["next_bop_test"] = None
    header["last_accident"] = _date_parse(_cell(ws, 39, 15, L))
    header["mud_type"]    = _clean(_cell(ws, 3, 25, L)) or _clean(_cell(ws, 3, 26, L))
    header["supervisor"]  = _clean(_cell(ws, 41, 26, L) or "")
    header["water_truck"] = 0
    td_raw = _clean(_cell(ws, 2, 21, L) or "")
    header["well_md"] = _float(td_raw) if td_raw else 0.0
    csg_label = _cell(ws, 2, 11, L)
    csg_depth = _cell(ws, 2, 12, L)
    if csg_label or csg_depth:
        header["top_shoe"] = _format_csg(str(csg_label or ""), csg_depth)
    liner_label = _cell(ws, 2, 14, L)
    liner_depth = _cell(ws, 2, 16, L)
    ...
    for key, r, c in [("density",4,26),("fun_vis",11,24),("yp",11,26),
                      ("solid",14,24),("ph",14,26),("pv",15,26)]:
        val = _mc(r, c)
        if val is not None:
            mud_checks[key] = val
    gel_raw = _clean(_cell(ws, 13, 26, L) or "")
    if gel_raw:
        mud_checks["gel10sec"] = gel_raw
    oil_water_raw = _clean(_cell(ws, 15, 24, L) or "")
    if oil_water_raw:
    ...
    mud_volume = {}
    for row, key in [(5,"mud_made"),(6,"trip_loss"),(7,"transfer"),
                     (8,"well_volume"),(9,"total_volume")]:
        v = _cell(ws, row, 26, L)
        if v is not None:
    ...
    for row in range(19, 26):
        ...
        if not name:
            continue
        qty = _clean(_cell(ws, row, 26, L) or "")
        chemicals.append({"item": name, "units": "", "received": "", "used": qty, "on_loc": ""})

    _SKIP = {"PERSONNEL"}
    personnel = []
    for row in range(34, 43):
        company = _clean(_cell(ws, row, 23, L) or "")
        if not company or company.upper() in _SKIP:
            continue
        count = _cell(ws, row, 26, L)
        personnel.append({"company": company, "number": _int(count, 0), "hours": "", "names": ""})
    ...
    for row in range(18, ws.max_row + 1):
        ...
        if any(m in a_text for m in ("SITUATION","PROGRAMME","REMARQUE","BUT DU WORK")):
            break
        if desc.upper() == "AFTER MIDNIGHT" and a is None and b is None:
            continue
    ...
    tarif_totals = {}
    for code, col in [("T1",19),("T2",20),("T3",21),("NR",22)]:
        v = _cell(ws, 16, col, L)
        if v is not None:
            f = _float(v)
            if f > 0:
    ...
    text_sections = {}
    prog_parts = []
    for row in range(43, min(ws.max_row + 1, 53)):
        v = _clean(_cell(ws, row, 3, L) or "")
        if v and v.strip() not in ("", " "):
            prog_parts.append(v)
    if prog_parts:
        text_sections["plan_operations"] = " ".join(prog_parts)
    obs_parts = []
    for row in range(28, 32):
        for col in range(15, 23):
            v = _clean(_cell(ws, row, col, L) or "")
            if v and v.strip() not in ("", " "):
                obs_parts.append(v)
                break
    if obs_parts:
        text_sections["remarks"] = " ".join(obs_parts)

    safety = {}
    if header.get("bop_test"):
        safety["bop_test"] = header["bop_test"]
    if header.get("last_accident"):
        safety["last_accident_date"] = header["last_accident"]
    ex_status = _clean(_cell(ws, 35, 20, L) or "")
    if ex_status:
        safety["exercises"] = ex_status
