#!/usr/bin/env python3
"""
gwdc155_extract.py — extract a GWDC-155 (rig) / HRLC-33 (well) / HRM
(field) "COMPLÉTION" (completion) Daily Report from a PDF into the
standard dict shape.

Source layout
-------------
Same Excel-exported-to-PDF template family as tp212/enf03/tp217/tp237,
titled "RAPPORT JOURNALIER COMPLÉTION" rather than "WORK OVER" or
"DTM". Activities are again rebuilt from raw word coordinates rather
than the grid (the time-band cell has exactly as many lines as real
activities while the description cell has extra wrapped-continuation
lines with no blank placeholder at the same index — the same reason
tp212/enf03/tp217/tp237 all need the word-position parser instead of a
simple newline-zip).

Real differences from the sibling reports, accounted for here:

  - Well construction has only TWO shoe references — "Sabot 9\" 5/8"
    (-> last_csg_shoe) and "Sabot 7\"" (-> last_lnr_shoe) — plus a new
    "Fond 7\"" field (the 7" string's current/total depth reference,
    not a liner TOP), which gets its own `fond_7_depth`/`fond_7_size`
    fields rather than being forced into last_lnr_top.

  - A mud-tank density reading ("Bac SH OBM  d=1,40") sits in the
    HEADER area itself (row 2, before "TARIFICATION"), outside the
    normal mud-checks scan window — captured explicitly and folded
    into mud_checks as `bac_sh_obm_density`.

  - Cost breakdown includes new company names not seen in the sibling
    reports ("PLATE-FORME GTP", "TRANSPORT EMEC") — handled for free
    since that loop is generic (label-based, not a hardcoded list).

  - The water/GDR section uses "Eau incd Rig" (incendie/fire water)
    alongside "Eau-Rig" — handled for free by the generic water scan.

Distinguishing markers (for helpers.parse_source._detect_format):
    - file extension .pdf, Creator "Microsoft Excel" / "Microsoft® Excel®"
    - contains "RAPPORT JOURNALIER" + "COMPLÉTION" + "APPAREIL: GWDC 155"
      (or generally "PUITS: HRLC" + "CHAMP: HRM")
"""
...
def _repair_mud_pair(label: str, value: str):
    """Fix specific Excel-export artifacts seen in this report family
    where a long label gets cut mid-token at the cell boundary and its
    remainder bleeds into the START of the value cell, e.g.
        label='Réception OBM d=0.'  value='9 0'
        -> label='Réception OBM d=0.9', value='0'
        label='V. S Bacs SH (gas-oil'  value=') 27'
        -> label='V. S Bacs SH (gas-oil)', value='27'
        label='Oil / Water rati'  value='o 90/10'
        -> label='Oil / Water ratio', value='90/10'
    Handles: an unterminated decimal point ("...d=0.") pulling a leading
    digit run off the value cell; an unterminated open-parenthesis
    pulling a leading ")" off the value cell; and a plain word split
    (lowercase letters) pulling the rest of the label's last word off
    the value cell. Falls back to discarding a stray leftover token in
    an otherwise-plain value (e.g. "0 0" -> "0") since these panels
    only ever hold single short tokens.
    """
    ...
    # A plain WORD split mid-token across the cell boundary, e.g.
    # "Oil / Water rati" / "o 90/10" (the label was cut off mid-word,
    # and the value cell's leading letters complete it) — checked before
    # the generic stray-token fallback below, which would otherwise
    # mistake the completing fragment ("o") for the whole value.
    word_split = re.match(r"^([a-zà-ÿ]+)\s+(.*)$", value)
    ...
    elif word_split and re.search(r"[a-zà-ÿ]$", label):
        label = label + word_split.group(1)
        value = word_split.group(2).strip()
    else:
...
# Main extractor
# ---------------------------------------------------------------------------
def parse_gwdc155(source: Union[Path, str]) -> dict:
    """Parse a GWDC-155 / HRLC-33 (PDF) Completion report."""
    ...
    with pdfplumber.open(str(source_path)) as pdf:
        ...
        # This report has only two shoe references plus a new "Fond 7''"
        # field (a current/total-depth reference for the 7" string, not
        # a liner TOP — kept as its own field rather than last_lnr_top).
        ...
        # Standardized last_csg_shoe / last_lnr_shoe (size@depth, matching
        # the enf17 DDR extractor's naming/format).
        ...
        # New to this report: "Fond 7''" — current/total depth reference
        # for the 7" string (not a liner top), its own field.
        if header.get("fond_7"):
            size, depth = _parse_size_depth(header["fond_7"])
            if size:
                header["fond_7_size"] = size
                if depth is not None:
                    header["fond_7_depth"] = depth
        ...
        if tarification_pos and produits_title_pos:
            ...
            for r in range(r0, r1):
                ...
                for lab_c, val_c in ((mud_col, mud_col + 1), (mud_col + 2, mud_col + 3)):
                    ...
                    vs = _clean(val)
                    if vs == "": continue
                    # Some values are written as "d=1,40" rather than a bare
                    # number (e.g. the header-area "Bac SH OBM  d=1,40"
                    # mud-tank density reading) — strip that prefix so it
                    # parses as a clean float instead of a raw string.
                    dm = re.match(r"^d\s*=\s*([\d.,]+)$", vs, re.IGNORECASE)
                    if dm: vs = dm.group(1)
                    fv = _float(vs)
                    # Keep genuinely non-numeric values (e.g. "95/05") as text
                    mud_checks[key] = fv if re.match(r"^[\d.,\-]+$", vs) else vs
...
# Drop-in compat
parse_daily_pdf_report = parse_gwdc155


if __name__ == "__main__":
    import sys, json
    if len(sys.argv) < 2:
        sys.exit("Usage: gwdc155_extract.py SOURCE.pdf")
    data = parse_gwdc155(Path(sys.argv[1]))
