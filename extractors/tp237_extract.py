#!/usr/bin/env python3
"""
tp237_extract.py — extract a TP-237 (rig) / HR-128 (well) / HRM (field)
"DTM" (rig-move / démontage) Daily Report from a PDF into the standard
dict shape.

Source layout
-------------
Same Excel-exported-to-PDF template family as tp212/enf03/tp217, but a
"DTM" variant used on rig-move days rather than active workover days —
titled "RAPPORT JOURNALIER DTM" with a "DTM N°" day-number label instead
of a bare number. Being a rig-move day, almost every numeric field on
this report is blank (no tarification hours, no daily/cumulative cost,
no mud checks, no chemical usage, no personnel counts) — the extractor
must treat "blank" as genuinely-no-data rather than 0, which the
generic label/value reading it inherits already does correctly.

Real structural differences from tp212/enf03/tp217 accounted for here:

  - The ENTIRE grid is shifted one column to the left versus the other
    three reports (14 grid columns here vs. 15 there). Every part of
    this extractor that finds columns by searching for label TEXT
    (which is most of it) is unaffected; the two spots that used a
    fixed relative offset (mud-checks column, described below) already
    compute that offset relative to a located label rather than an
    absolute index, so they adapt automatically.

  - The mud-checks panel's first row sits ONE ROW BEFORE the
    "TARIFICATION" cell (TP-212/ENF-03 start the row after;
    TP-217 starts on the same row; this one starts a row earlier).
    Rather than add a fourth special case, the scan window is widened
    to start 2 rows before "TARIFICATION" — any row in that wider
    window that doesn't actually hold mud-panel content in those
    columns is silently skipped, so widening the window is free.

  - The chemicals table's header row has NO "Produits" text this time
    (that cell is blank; only "Unité"/"Utilisés"/"Stock" are present),
    so the primary "Produits"+"Unité"-together match falls back to
    "the row right after the PRODUITS section title" when it fails.

  - The water section uses different, shorter labels ("Eau" / "OBM
    Bacs SH" instead of "Eau-Rig" / "Eau-Dinc-SH+ENTP" / ...), so the
    water-section anchor tries "Eau-Rig" first and falls back to the
    bare "Eau" label.

  - Well construction has FOUR distinct size/depth references instead
    of two or three: "Sabot 7''", "Sabot 4''1/2" (a second, smaller
    liner run), "TOL 4''1/2" (Top Of Liner) and "Top Fond 4''1/2" (top
    of the fish/junk being fished for — new to this report). "Sabot
    7''"/"TOL 4''1/2" map onto the established last_lnr_shoe/
    last_lnr_top fields; "Sabot 4''1/2" and "Top Fond 4''1/2" have no
    precedent in the earlier reports, so they get their own clearly
    named fields (`scab_liner_shoe*`, `top_fond*`) rather than being
    forced into an existing slot.

  - Bug fix (applies generally, not just to this report): the VEHICULE
    field lookup used to grab the FIRST truthy cell anywhere to the
    right of the "VEHICULE" label, unbounded. In every report of this
    family, "VEHICULE" and the Emetteur/Récepteur NAMES happen to sit
    on the same grid row (the row right after the "Emetteur
    (Chantier)" label row) — TP-212/ENF-03/TP-217 got lucky because
    the vehicle-type cell right after "VEHICULE" was always filled in
    (e.g. "HILUX"), so the search never had to look past it. Here the
    vehicle-type cell is blank, so the old unbounded search kept going
    and incorrectly grabbed the Emetteur's own name instead. Fixed by
    bounding the search to stop before the Emetteur name's column.

  - Bug fix: the day-number cell here reads "DTM N°\\n1" (label AND
    number in one cell) rather than a bare number, so day-number
    extraction now pulls the trailing digits out of that cell instead
    of requiring the whole cell to be numeric.

As with the sibling extractors, the single activity in "ANALYSE DES
TEMPS ET DES OPERATIONS" is rebuilt from raw word coordinates, and the
description-column right edge falls back to a fixed default when (as
here) there's no "Tarif" header word to anchor on dynamically.

Distinguishing markers (for helpers.parse_source._detect_format):
    - file extension .pdf, Creator "Microsoft Excel" / "Microsoft® Excel®"
    - contains "RAPPORT JOURNALIER" + "DTM" + "APPAREIL: TP 237"
      (or generally "DTM N°" + "PUITS: HR" + "CHAMP: HRM")
"""
...
# Main extractor
# ---------------------------------------------------------------------------
def parse_tp237(source: Union[Path, str]) -> dict:
    """Parse a TP-237 / HR-128 (PDF) DTM (rig-move) report."""
    ...
    with pdfplumber.open(str(source_path)) as pdf:
        ...
        if pos:
            ...
            # Usually a bare number ("76"), but on DTM reports the cell
            # reads "DTM N°\n1" (label AND number together) — pull the
            # trailing digits either way.
            if day_val:
                m = re.search(r"(\d+)\s*$", _clean(day_val))
        ...
        # "Tête Liner 7''", "TOL 4''1/2", "Sabot 7''", "Sabot 4''1/2",
        # "Bridge Plug 7''", "Top Fond 4''1/2", "KOP...").
        for prefixes, raw_key in [
            (["Sabot 9"], "sabot_9_5_8"),
            (["Sabot 7"], "sabot_7"),
            (["Sabot 4"], "sabot_4_1_2"),
            (["Top of Liner", "Tête Liner", "Tete Liner", "TOL"], "top_of_liner"),
        ...
        # New to this report: a second, smaller liner run's own shoe
        # ("Sabot 4''1/2") — no equivalent in the other reports, so it
        # gets its own field rather than overloading last_lnr_shoe.
        if header.get("sabot_4_1_2"):
            size, depth = _parse_size_depth(header["sabot_4_1_2"])
            if size:
                header["scab_liner_shoe_size"] = size
                if depth is not None:
                    header["scab_liner_shoe_depth"] = depth
                    header["scab_liner_shoe"] = f"{size} @ {depth:g}m"
        # New to this report: depth to the top of the fish/junk being
        # fished for ("Top Fond 4''1/2").
        if header.get("top_fond"):
            size, depth = _parse_size_depth(header["top_fond"])
            if depth is not None:
                header["top_fond_depth"] = depth
                header["top_fond_ref"] = size

        # BOUE type
        for r, row in enumerate(grid):
            for c, cell in enumerate(row):
                if cell and cell.upper().startswith("TYPE:"):
                    mt = _clean(cell.split(":", 1)[1])
                    if mt: header["mud_type"] = mt
        ...
        # Emetteur (Chantier) / Récepteur (Service) — located first because
        # the VEHICULE lookup below needs to bound itself against it.
        ...
        # Vehicule — bounded to stop before the Emetteur name's column.
        # "VEHICULE" and the Emetteur/Récepteur NAMES sit on the same grid
        # row across this whole report family (the row right after the
        # "Emetteur (Chantier)" LABEL row). An earlier version of this
        # extractor grabbed the first truthy cell anywhere to the right of
        # "VEHICULE" unbounded, which worked by luck on reports where the
        # vehicle-type cell (e.g. "HILUX") was filled in — but on this
        # report that cell is blank, so it kept scanning and incorrectly
        # picked up the Emetteur's own name instead.
        pos = _grid_find(grid, "VEHICULE")
        if pos:
            r, c = pos
            end_c = len(grid[r])
            if emet and emet[0] + 1 == r:
                end_c = emet[1]
            vals = [x for x in grid[r][c + 1:end_c] if x]
        ...
        if tarification_pos and produits_title_pos:
            # The mud panel's start row RELATIVE to "TARIFICATION" varies
            # by report instance: TP-212/ENF-03 start the row after,
            # TP-217 starts on the same row, and this DTM template starts
            # a row BEFORE it. Rather than special-case each one, widen
            # the scan window generously (2 rows before TARIFICATION) —
            # any row in that window with no real mud-panel content in
            # these columns is silently skipped, so widening is free.
            r0 = max(0, tarification_pos[0] - 2)
        ...
        # the plain "PRODUITS" section-title cell a few rows above). Falls
        # back to "the row right after the PRODUITS title" when the
        # "Produits" column-header text itself is blank, as it is here.
        # ================================================================
        chemicals = []
        prod_header = None
        for r, row in enumerate(grid):
            ...
            if "produits" in norm_cells and "unité" in norm_cells:
                ...
                prod_header = (r, c0)
                break
        if not prod_header and produits_title_pos:
            prod_header = (produits_title_pos[0] + 1, produits_title_pos[1])
        ceiling = len(grid)
        water_pos = _grid_find(grid, "Eau-Rig") or _grid_find(grid, "Eau")
        if water_pos:
            ceiling = water_pos[0]
        if prod_header:
            r0, c0 = prod_header
...
# Drop-in compat
parse_daily_pdf_report = parse_tp237


if __name__ == "__main__":
    import sys, json
    if len(sys.argv) < 2:
        sys.exit("Usage: tp237_extract.py SOURCE.pdf")
    data = parse_tp237(Path(sys.argv[1]))
