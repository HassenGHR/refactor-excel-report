#!/usr/bin/env python3
"""
enf10_extract.py — extract an ENF#10 (Haoud Berkaoui) Daily Workover Report
into the standard dict shape, following the same conventions as
enf04_extract.py.

Source layout
-------------
French workover template, sheet "rapport", ~71 rows.  Title at B11:
"RAPPORT JOURNALIER DE WORK OVER".  Regional direction at B8: "HAOUD
BERKAOUI".  This is the SAME underlying template as TP#215's report
(tp215_extract.py) — header block (rows 12-17) lines up exactly between
...
This file's parsing logic is identical to tp215_extract.py (verified
against both sample sheets) — the only differences observed were cosmetic
(e.g. rig name written as "ENF # 10" rather than "TP # 215", RAPPORT N°
given as a bare integer rather than a "RAPPORT N°:xxx" string), both of
which are already handled generically.
...
# Main extractor
# ---------------------------------------------------------------------------
def parse_enf10(source: Union[Path, str, BytesIO]) -> dict:
...
# Drop-in compat
parse_daily_excel_report = parse_enf10


if __name__ == "__main__":
    import sys, json
    if len(sys.argv) < 2:
        sys.exit("Usage: enf10_extract.py SOURCE.xlsx")
    data = parse_enf10(Path(sys.argv[1]))
